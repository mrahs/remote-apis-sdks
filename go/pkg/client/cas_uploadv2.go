...
func (c *Client) Upload(ctx context.Context, opt UploadOptions, inputC <-chan *UploadInput) (*UploadResult, error) {
	...
	u.checkBundler.BundleCountThreshold = u.CASConfig.FindMissingBlobs.MaxItems
	...
	u.batchBundler = bundler.NewBundler(&repb.BatchUpdateBlobsRequest_Request{}, func(subReq interface{}) {
		// Handle errors and context cancellation via errgroup.
		eg.Go(func() error {
	...
	u.batchBundler.BundleByteLimit = c.CASConfig.BatchUpdateBlobs.MaxSizeBytes - int(marshalledFieldSize(int64(len(c.InstanceName)))) - 1000
	u.batchBundler.BundleCountThreshold = c.CASConfig.BatchUpdateBlobs.MaxItems
...
func (u *uploader) visitRegularFile(ctx context.Context, absPath string, info os.FileInfo) (*repb.FileNode, error) {
	isLarge := info.Size() >= u.CASConfig.LargeFileThreshold
	...
	if info.Size() <= u.CASConfig.SmallFileThreshold {
...
func (u *uploader) findMissingBlobs(ctx context.Context, items []*uploadItem) (res *repb.FindMissingBlobsResponse, err error) {
	...
	err = u.unaryRPC(ctx, &u.CASConfig.FindMissingBlobs, func(ctx context.Context) (err error) {
...
func (u *uploader) scheduleUpload(ctx context.Context, item *uploadItem) error {
	...
	if marshalledRequestSize(digest.NewFromProtoUnvalidated(item.Digest)) > int64(u.batchBundler.BundleByteLimit) {
...
func (u *uploader) uploadBatch(ctx context.Context, reqs []*repb.BatchUpdateBlobsRequest_Request) error {
	...
	return u.unaryRPC(ctx, &u.CASConfig.BatchUpdateBlobs, func(ctx context.Context) error {
...
func (u *uploader) stream(ctx context.Context, item *uploadItem, updateCacheStats bool) error {
	...
	return u.withRetries(ctx, func(ctx context.Context) error {
		...
		if u.CASConfig.CompressedBytestreamThreshold < 0 || item.Digest.SizeBytes < u.CASConfig.CompressedBytestreamThreshold {
			// No compression.
		...
		eg.Go(func() error {
			...
			case err == io.ErrClosedPipe:
				...
				// Might be a cache hit or context cancellation.
...
func (u *uploader) streamFromReader(ctx context.Context, r io.Reader, digest *repb.Digest, compressed, updateCacheStats bool) (rerr error) {
	ctx, cancel, withTimeout := withPerCallTimeout(ctx, u.CASConfig.ByteStreamWrite.Timeout)
	defer cancel()
