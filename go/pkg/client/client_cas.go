...
// CASClientConfig is a config for CAS Client.
// See DefaultCASClientConfig() for the default values.
...
func DefaultCASClientConfig() CASClientConfig {
...
func (c *CASClientConfig) Validate() error {
	switch {
...
func NewCASClient(ctx context.Context, conn *grpc.ClientConn, instanceName string) (*Client, error) {
	return NewCASClientWithConfig(ctx, conn, instanceName, DefaultCASClientConfig())
}

// NewClientWithConfig creates a new client and accepts a configuration.
func NewCASClientWithConfig(ctx context.Context, conn *grpc.ClientConn, instanceName string, config CASClientConfig) (*Client, error) {
	...
	if !client.CASConfig.IgnoreCapabilities {
...
// initCAS is a part of NewClientWithConfig that can be done in tests without
...
func (c *Client) initCAS() {
	c.semFindMissingBlobs = semaphore.NewWeighted(int64(c.CASConfig.FindMissingBlobs.Concurrency))
	c.semBatchUpdateBlobs = semaphore.NewWeighted(int64(c.CASConfig.BatchUpdateBlobs.Concurrency))
	c.semByteStreamWrite = semaphore.NewWeighted(int64(c.CASConfig.ByteStreamWrite.Concurrency))

	c.semFileIO = semaphore.NewWeighted(int64(c.CASConfig.FSConcurrency))
	c.semLargeFile = semaphore.NewWeighted(int64(c.CASConfig.FSLargeConcurrency))
	c.fileBufReaders.New = func() interface{} {
		return bufio.NewReaderSize(emptyReader, int(c.CASConfig.FileIOSize))
	}

	streamBufSize := 32 * 1024 // by default, send 32KiB chunks.
	if streamBufSize < c.CASConfig.ByteStreamWrite.MaxSizeBytes {
		streamBufSize = int(c.CASConfig.ByteStreamWrite.MaxSizeBytes)
	}
...
func (c *Client) withRetries(ctx context.Context, f func(context.Context) error) error {
	return retry.WithPolicy(ctx, retry.TransientOnly, c.CASConfig.RetryPolicy, func() error {
		return f(ctx)
	})
}
...
func (c *Client) checkCapabilities(ctx context.Context) error {
	caps, err := regrpc.NewCapabilitiesClient(c.CASConnection).GetCapabilities(ctx, &repb.GetCapabilitiesRequest{InstanceName: c.InstanceName})
	if err != nil {
	...
	if c.CASConfig.BatchUpdateBlobs.MaxSizeBytes > int(caps.CacheCapabilities.MaxBatchTotalSizeBytes) {
		c.CASConfig.BatchUpdateBlobs.MaxSizeBytes = int(caps.CacheCapabilities.MaxBatchTotalSizeBytes)
	}
