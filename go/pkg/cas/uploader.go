...
type BatchingUploader interface {
	...
	// The returned error wraps a number of errors proportional to the length of the specified slice.
	Upload(context.Context, []ep.Abs, slo.Options, *walker.Filter) ([]digest.Digest, *Stats, error)
...
type StreamingUploader interface {
	...
	Upload(context.Context, <-chan ep.Abs, slo.Options, *walker.Filter) <-chan UploadResponse
